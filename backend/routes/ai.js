...
async function callGroq(prompt) {
  ...
  throw new Error(data.error ? data.error.message : 'Unknown error from Groq');
}
...
router.post('/analyze-code', async (req, res) => {
  try {
    const { code, language = 'javascript' } = req.body;
    const result = await callGroq(`Analyze this ${language} code for bugs and improvements:\n\n${code}`);
...
router.post('/generate-docs', async (req, res) => {
  try {
    const { code, language = 'javascript' } = req.body;
    const result = await callGroq(`Generate documentation for this ${language} code:\n\n${code}`);
